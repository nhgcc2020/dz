<template>
    <div>
        ...
        <div class="big_first">
            <router-link tag="div" @click.native="hunt('Adressbook')" to="/seek" class="seek">
            ...
            <ul class="ul_two">
                <li v-for="(n,index) in news" key="index" v-if="n.partnerKey!=='temp-customer'">
                    ...
                    <router-link @click.native="jump(n,'Adressbook')" tag="div" to="/friend" style="position:absolute; top:0;height:90%;width:100%;z-index:100;background: transparent;"></router-link>
                </li>
            </ul>
...
export default {
    ...
    created() {
        ...
        JAnalytics.onPageStart({'pageName': '客户列表页'});
    },
    destroyed(){
        JAnalytics.onPageEnd({'pageName': '客户列表页'});
    },
    methods: {
        getInfo(type) {
            var _this = this
