<template>
  <div>
    <div class="x-header">
      <back-header>
        <p class="title">销售单详情</p>
        <i class="iconfont sare" style="right:50px;" @click="ation">&#xe6e9;</i>
        <router-link tag="i" :to="'/billingAmend/'+$route.params.salesNo" class="iconfont sare">&#xe6e6;</router-link>
      </back-header>
    </div>
    <div class="big_first">
      <ul class="ul_one">
      ...
      <ul class="ul_fore">
        ...
        <li v-if="news.isReceived==1">发货状态 :&nbsp;&nbsp; <span style="color:#01a09a;">对方已收货&nbsp;&nbsp;{{news.receivedTime}}</span></li>
        <li>销售单号 :&nbsp;&nbsp; {{news.salesNo}}</li>
        <li v-if="news.purchaseNo" style="color:#adadad;">好友采购单号 : {{news.purchaseNo}}</li>
      </ul>
      <div style="text-align: center;padding:1rem 0;color:#ccc;font-size:.8rem;">——我是有底线的——</div>
      <div style="opacity:0;" id="id_text">{{url.shareUrl}}</div>
      <div class="alt">
        <p>系统版本过低请手动复制</p>
        <p>{{url.shareUrl}}</p>
        <p @click="know">关闭</p>
  ...
  methods: {
    getInfo() {
      ...
      this.axios.post(address.particulars + localStorage.getItem('token'), params)
        .then(function (data) {
          _this.url = data.data
          _this.news = data.data.saleOrder;
