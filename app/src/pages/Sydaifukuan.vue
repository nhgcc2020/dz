<template>
  <div style="overflow: hidden;">
    <div class="x-header">
      <back-header>
        <p class="title">采购对账单列表</p>
      </back-header>
    </div>
    <ol class="ol_quit">
      <li class="ol_line" @click="bill(0)"><span>待付款</span></li>
      <li @click="bill(1)"><span>已付款</span></li>
    </ol>
    <div class="quit_time">
    ...
    <div class="anchor">
      <ul class="anchor_ul">
        <li patk="" class="anchor_color" @click="keydown(0)">全部供应商</li>
   ...
   <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData">
      <div>
        <div class="sare_time" v-for="(n,index) in news" key="index">
          <router-link tag="div" :to="'/porderchecking/'+ n.purchasecheckingNo">
            <ul class="sare_ul">
              <li class="sare_one" v-cloak><i class="iconfont">&#xe6c3;</i>&nbsp;&nbsp;<span>{{n.partnerShortname}}</span></li>
              <li class="sare_two" v-cloak><span>{{n.orderCycle}}</span><span>{{(n.receivablesMoney).toFixed(2)}}</span></li>
  ...
  methods: {
    getTime() {
     ...
     getInfo(pageSize, pageNo,Status,startTime,endTime,partnerKey) {
      ...
      this.axios.post(address.reportthree + localStorage.getItem('token'), params)
        .then(function (data) {
