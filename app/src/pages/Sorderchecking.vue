<template>
  <div>
    ...
    <div class="big_first"style="overflow: hidden;">
      <ul class="ul_one">
        <li class="li_one">
          <span style="flex:1;">{{news.partnerShortname}}</span>
        ...
        <li class="li_two">
          ...
          <span style="color:#999;font-size:12px;" v-cloak><i style="font-size:22px;border:1px solid #f2f3f5;border-radius:50%;position:relative;top:3px;" class="iconfont">&#xe6f4;</i>&nbsp;&nbsp; 应收&nbsp; <span style="font-family: 'arial';font-size:18px;color:#ff6600;position:relative;top:2px;">{{news.receivablesMoney | money}}</span></span>
        </li>
        <li v-if="news.remarks" class="li_three">备注 : {{news.remarks}}</li>
      </ul>
      <div id="dingdan" v-for="(d,index) in datasumber" key="index">
        <div class="time"><span><i class="iconfont">&#xe67b;</i>&nbsp;<span>{{d[0].salesNo}}</span>&nbsp;<span v-if="d[0].isReceived==1">已收货</span></span><span><i class="iconfont">&#xe6f3;</i>&nbsp;<span>{{d[0].salesOrderDate}}</span></span></div>
        <ul class="ul_two" v-for="(l,index) in datasumber[index]" key="index">
          ...
          <li style="color: #999;padding-bottom: 0.5rem;border-bottom:1px solid #f9f9f9;font-size:12px;display:flex;" v-cloak>
            ...
            <span style="float:right;color:#eb6877;margin-right:1.1rem;">{{l.totalPrice | money}}</span></li>
        </ul>
      </div>
      <ul class="ul_three">
        ...
        <li class="li_right" style="color:#adadad;">&nbsp;&nbsp;&nbsp;&nbsp;预收
          <span v-cloak>- {{news.depositRequested | money}}</span>
        </li>
         <li style="border-top:1px solid #F6F6F6;line-height:44px;display:flex;font-size:14px;">
          <span style="color:#000;flex:1;" v-cloak>应收金额</span>
      ...
      <div style="margin-left:1000px;" id="id_text">{{url.shareUrl}}</div>
      <div class="alt">
               <p>系统版本过低请手动复制</p>
               <p>{{url.shareUrl}}</p>
               <p @click="know">关闭</p>
...
export default {
  ...
  methods: {
    getInfo() {
      var _this = this;
      var params = "salescheckingNo=" + this.$route.params.salesNo;
      // params.append('salesNo',this.$route.params.salesNo);
      this.axios.post(address.sorderchecking + localStorage.getItem('token'), params)
        .then(function (data) {
          //  debugger;
          _this.url = data.data;
          _this.news = data.data.checkingOrder;
