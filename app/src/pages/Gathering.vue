    ...
    <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData">
      <div>
        <div class="sare_time" v-for="(n,index) in news" key="index">
          <router-link @click.native="relation(n.partnerKey)" tag="div" to="/sdaishoukuanone">
            <ul class="sare_ul">
              <li class="sare_one" v-cloak>
                <div>
                  <p>{{n.partnerName}}</p>
                  <p><span style="color:red; font-size:18px;">{{n.totalPrice07 | money}}</span>&#x3000;<span style="color:#989898;">{{n.orderCount}}笔</span></p>
                </div>
...
export default {
  ...
  methods: {
    ...
    getInfo(pageSize, pageNo,value1,value2,sort,partnerKey) {
      var _this = this;
      var params = "pageSize=" + pageSize + "&pageNo=" + pageNo + "&orderStatus=07&startTime="+value1+"&endTime="+value2+"&order="+sort+"&partnerKey="+partnerKey;
