<template>
  <div style="overflow: hidden;">
    <div class="x-header">
      <back-header>
        <p class="title">销售对账单列表</p>
      </back-header>
    </div>
    <ol class="ol_quit">
      <li class="ol_line" @click="bill(0)">
        <span>待收款</span>
      </li>
      <li @click="bill(1)">
        <span>已收款</span>
    ...
    <div class="quit_time">
      <span @click="client" :class="{ anchor_col: !show3 }">{{clientele | character}}
        <i v-if="show3" class="iconfont" style="font-size:14px;margin-left:4px;">&#xe68d;</i>
        <i v-if="!show3" class="iconfont" style="font-size:14px;margin-left:4px;color:#4284d9;">&#xe68e;</i>
      </span>
      <span @click="select" :class="{ anchor_col: !show2 }">
        <span>{{value1}}</span> -
        <span>{{value2}}</span>
        <i v-if="show2" class="iconfont" style="font-size:14px;margin-left:4px;">&#xe68d;</i>
        <i v-if="!show2" class="iconfont" style="font-size:14px;margin-left:4px;color:#4284d9;">&#xe68e;</i>
      </span>
    </div>
    <div class="anchor">
      <ul class="anchor_ul">
    ...
    <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData">
      <div>
        <div class="sare_time" v-for="(n,index) in news" key="index">
          <router-link tag="div" :to="'/sorderchecking/' + n.salescheckingNo">
            <ul class="sare_ul">
              ...
              <li class="sare_two" v-cloak>
                <span>{{n.orderCycle}}</span>
                <span>{{(n.receivablesMoney).toFixed(2)}}</span>
              </li>
...
export default {
  ...
  created() {
    ...
    JAnalytics.onPageStart({ 'pageName': '销售对账单' });
  },
  destroyed() {
    JAnalytics.onPageEnd({ 'pageName': '销售对账单' });
  },
  methods: {
    getTime() {
      var _this = this;
    ...
    getInfo(pageSize, pageNo, Status, startTime, endTime, partnerKey) {
      ...
      this.axios.post(address.reportone + localStorage.getItem('token'), params)
        .then(function (data) {
    ...
    bill(index) {
      $(".ol_quit li").eq(index).addClass("ol_line").siblings().removeClass("ol_line");
      if (index == 0) {
        this.news = [];
        this.Status = "07";
        this.showding = true;
    ...
    initial() {
      ...
      if (this.render) {
        ...
        $(".ol_quit li").eq(0).addClass("ol_line").siblings().removeClass("ol_line");
        this.getTime();
        this.getInfo(this.pageSize, this.pageNo, this.Status, this.value1, this.value2, this.matter);
      }
    }
