...
export default {
  ...
  data() {
    return {
        ...
        value1:"",
        show:false,
        uldate:[{"businessDesc":"","length":"","width":"","unitPrice":"","quantity":"","totalPrice":0,"priceWay": "02","priceWayDesc": "按数量"}],
        operator:[],
        index:"",
        comKey:"",
        scrollY:"",
  ...
  methods: {
    ...
    construction(){
        ...
        this.disabled = true;
        var saleOrder = {};
        saleOrder.contactId = this.contacterId;
        saleOrder.contactName = this.contacterName;
        saleOrder.partnerShortname = this.responsible;
        saleOrder.partnerKey = this.comKey;
        saleOrder.orderDate = this.value;
        saleOrder.caseDesc = this.caseDesc;
        saleOrder.price = $("#dM").text();
        saleOrder.totalPrice = $("#dM").text();
         this.axios.post(address.billing + localStorage.getItem('token'), { "salesOrder": saleOrder, "details": this.uldate }, { headers: { 'Content-Type': 'application/json' } })
          .then(data => {
            if(data.data.status == '1'){
                $(".error_tips").html("开单成功").fadeIn(250).delay(500).fadeOut(250);
                setTimeout(function() {
                    this.$router.go(-1);
                    bus.$emit('save');
                }.bind(this), 1000);
