<template>
  <div style="overflow: hidden;">
    <div class="x-header">
      <back-header>
        <p class="title">工资账本</p>
        <router-link tag="p" class="remember" :to="'/newaccount/'+'工资账本'"><i class="iconfont" style="font-size:25px;float:left;">&#xe6a1;</i>&nbsp;记一笔</router-link>
      </back-header>
    </div>
    <div class="quit_time">
    ...
    <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData">
        ...
        <div>
          <ul class="sare_ul">
            <router-link tag="li" :to="{ name: 'update', params: {'expenseType':'1','expenseNo':n[0].expenseNo}}" class="sare_one" v-cloak><span>{{n[0].payDate}}</span><span>{{n[0].spenderName | spenderName}}</span><i class="iconfont" style="color:#999;font-size:14px;">&#xe68b;</i><span>&yen; {{n[0].payType | pattern}}</span></router-link>
 ...
 directives: {
  ...
  methods: {
    ...
    getInfo(pageSize, pageNo,startTime,endTime,Detail) {
      var _this = this;
      var params = "pageSize="+pageSize+"&pageNo="+pageNo+"&expenseType=1&startTime="+startTime+"&endTime="+endTime+"&expenseTypeDetail="+Detail;
      this.axios.post(address.dayaccount + localStorage.getItem('token'), params)
        .then(function (data) {
          ...
          var expenseNo = data.data.rows[0].expenseNo;
          var arr = [];
