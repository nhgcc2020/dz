<template>
  <div style="overflow: hidden;">
    ...
    <div class="quit_time">
      ...
      <span @click="client" :class="{pitch:show1}">{{clientele}}
    ...
    <div class="tasking">
      <p @click="checked(0,'')" class="pitch">全部供应商</p>
      <p @click="checked(index+1,p.partnerKey)" v-for="(p,index) in operator" key="index">{{p.partnerShortname}}</p>
    </div>
    <ul class="anchor">
      <li @click="stage(0)" class="pitch">待对账</li>
      <li @click="stage(1)">已对账</li>
    </ul>
    <div class="side">
      <ul>
    ...
    <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData" style=" top: 96px;">
      <ul v-for="(n,index) in news" key="index" class="ul_count">
        <router-link tag="li" :to="'/porderchase/' + n.purchaseNo" style="border-bottom:1px solid #f2f2f2;display:flex;">
          <div style="flex:1;">
            ...
            <p style="position:relative;margin:-5px 0 10px 0;">
              <span style="font-size:12px;color:#ccc;">采购日期</span>&nbsp;
        ...
        <li>
          <p style="margin:10px 0;">{{n.caseDesc | casedesc}}</p>
        </li>
        <li style="text-align:right;font-size:12px;color:#ccc;margin-bottom:13px;">共 &nbsp;&nbsp;
          <span style="color:#666;">{{n.detailNum}}</span> &nbsp;&nbsp;条明细</li>
        <li v-if="n.workingStatus=='5'" style="font-size:12px;color:#666;border-top:1px solid #f2f2f2;height:40px;display:flex;align-items:center;">
           <router-link tag="div" :to="'/syreportDetails/' + n.salesNo" style="flex:1;position:relative;color:#ff9900;font-size:12px;"><i style="font-size:30px;position:absolute;top:-13px;" class="iconfont">&#xe6f7;</i>&#x3000;&#x3000;&#x3000;<span>外协制作中</span><i style="color:#999;font-size:14px;margin-left:10px;position:relative;top:1px;" class="iconfont">&#xe68b;</i> </router-link> 
           <div><span style="position:relative;top:-2px;margin-right:10px;">订单金额</span><span style="font-family: 'arial';font-size:18px;color:#ff6f10;">{{n.totalPrice | filmoney}}</span></div>
        </li>
        <li v-if="n.workingStatus=='2'" style="font-size:12px;color:#666;border-top:1px solid #f2f2f2;height:40px;display:flex;align-items:center;">
           <router-link tag="div" :to="'/syreportDetails/' + n.salesNo" style="flex:1;position:relative;color:#01a09a;font-size:12px;"><i style="font-size:30px;position:absolute;top:-13px;" class="iconfont">&#xe6f7;</i>&#x3000;&#x3000;&#x3000;<span>外协已完工</span><i style="color:#999;font-size:14px;margin-left:10px;position:relative;top:1px;" class="iconfont">&#xe68b;</i> </router-link> 
           <div><span style="position:relative;top:-2px;margin-right:10px;">订单金额</span><span style="font-family: 'arial';font-size:18px;color:#ff6f10;">{{n.totalPrice | filmoney}}</span></div>
        </li>
        <li v-if="!n.workingStatus" style="font-size:12px;color:#666;border-top:1px solid #f2f2f2;height:40px;display:flex;align-items:center;justify-content: flex-end;">
            <span style="position:relative;top:-0px;margin-right:10px;">订单金额</span><span style="font-family: 'arial';font-size:18px;color:#ff6f10;">{{n.totalPrice | filmoney}}</span>
        </li>
      </ul>
    </v-scroll>
    <div class="foot">
...
export default {
  ...
  created() {
    this.getTime();
    this.getInfo(this.pageSize, this.pageNo, this.Status, this.value1, this.value2, this.salesId);
  },
  mounted() {
    if (this.count == "待对账") {
      $(".quit_time>span").eq(0).css("color", "#f26628");
    } else {
      $(".quit_time>span").eq(0).css("color", "#666666");
    }
  },
  methods: {
    getTime() {
    ...
    getInfo(pageSize, pageNo, Status, startTime, endTime, partnerKey) {
      ...
      this.axios.post(address.reporttwo + localStorage.getItem('token'), params)
        .then(function (data) {
    ...
    stage(i) {
      ...
      if (i == 0) {
        ...
        $(".quit_time>span").eq(0).css("color", "#666");
        this.Status = "05";
      }
      this.showding = true;
      this.news = [];
