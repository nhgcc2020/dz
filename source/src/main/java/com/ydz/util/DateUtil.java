...
public class DateUtil {
     ...
     * @return int
     * @throws 
     * @Description: 计算两个日期的相差月数
     */
    public static int getIntervalMonths(Date start, Date end) throws ParseException{
    	if (start.after(end)) {
            Date t = start;
            start = end;
            end = t;
     ...
     * @return int
     * @throws 
     * @Description: 根据月份算总天数
     */
    public static int getDayByMonth(int month){
    	BigDecimal day = null;
    	if(month>0){
    		 day = new BigDecimal(365).divide(new BigDecimal(12),5, RoundingMode.HALF_UP).multiply(new BigDecimal(month)).setScale(0,BigDecimal.ROUND_HALF_UP);
    	}
    	return day.intValue();
    }
    /**
