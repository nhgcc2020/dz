...
/** 
 * @ClassName: AppReportChartController 
 * @Description: app的报表图表控制器
 * @author Bruce 赵石磊 4stones@yiduizhang.com
 * @date 2017年6月9日 下午4:55:00 
 * 
 */
@RestController
@RequestMapping(value = "app/reportchart", produces = "application/json;charset=UTF-8")
public class AppReportChartController extends BaseController{
	@Resource
	...
	public String getSalesTrend(HttpServletRequest request) throws Exception{
		...
		resultMap.setTotalCount(amounts.size());
		resultMap.setRows(amounts);
		
		return JsonUtil.toCompatibleJSONString(resultMap);
	}
	
	@RequestMapping(value="salestotalpricebench",method = RequestMethod.POST)
	public String totalPriceBench(HttpServletRequest request) throws Exception{
		UserVo userVo = this.getAppCurrentUserVo();
		Page page = new Page();
	...
	@RequestMapping(value="salestotalpricelist",method = RequestMethod.POST)
	public String totalPriceList(HttpServletRequest request) throws Exception{
		UserVo userVo = this.getAppCurrentUserVo();
		...
		resultMap.setTotalCount(page.getTotalPage());
		return JsonUtil.toCompatibleJSONString(resultMap);
	}
	
	@RequestMapping(value="salesOrderPartnerAmount",method = RequestMethod.POST)
	@ResponseBody
...
//			}
//		}
//		if ("sumTotalPrice".equalsIgnoreCase(pd.getString("sort"))) {
//			if ("asc".equalsIgnoreCase(pd.getString("order"))) {
...
//		}
		...
		resultMap.setRows(amounts);
		resultMap.setTotalCount(page.getTotalPage());
		resultMap.put("totalRows", page.getTotalResult());
		resultMap.put("sumTotalPriceAll", sumTotalPrice);
		return JsonUtil.toCompatibleJSONString(resultMap);
	}
	
	@RequestMapping(value="companyRecPayAmounts",method = RequestMethod.POST)
	public String getCompanyRecPayAmounts() throws Exception{
		UserVo userVo = this.getAppCurrentUserVo();
		PageData pd = this.getPageData();
		pd.put("comKey", userVo.getComKey());
		pd.put("startDate", pd.getStartDate(pd.getString("startTime")));
	...
	@RequestMapping(value="gross-profit",method = RequestMethod.POST)
	public String getCompanySheet() throws Exception{
		UserVo userVo = this.getAppCurrentUserVo();
		PageData pd = this.getPageData();
		pd.put("comKey", userVo.getComKey());
