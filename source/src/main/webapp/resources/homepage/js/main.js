$(document).ready(function(){
	...
	(function(){
		var num = 0 ;
		var w=$(".scroll_list").width();
		$(".scroll_list ul").width(w*30);
		var scroll_time = setInterval(banner_cscroll,4000);
		function banner_cscroll(){
			num++;
			if(num==30){
				num = 0;
			}
		...
		//鼠标悬停
		$('.scroll_list').mousemove(function(){
			clearInterval(scroll_time);
		});
		//鼠标移出
		$('.scroll_list').mouseleave(function(){
			scroll_time = setInterval(banner_cscroll,4000);
		});
	})();
	//注册
