jQuery(function(){
    ...
    // 缩略图大小
    // var thumbnailWidth = window.webuploader.config.thumbWidth || 'auto';
    ...
    var uploader = WebUploader.create({
        ...
        pick: {
            id:'#filePicker',
            label:window.webuploader.config.btnName|| '点击添加图片'
        },
        dnd: '.queueList',
        paste: document.body,
        accept:{
    ...
    function removeServerFile( file ){
        ...
        $.ajax({
            type:'DELETE',
            url: window.webuploader.removeUrl+"?fileId="+file.key,
            dataType:'json',
            success:function(data){
                if(data.code=="1"){
                    $('#'+file.id).remove();
                    ...
                    $info.text("删除失败").appendTo( $("#"+file.id) );
                }
    ...
    function initServerFile(){
        if( window.webuploader.initUrl ){
            ...
            // var data=[{"src":"/resources/easyui/images/icons.png","path":"/images/05_5.png","size":33,"key":"","id":"24234234","name":"所属","statusText":""}]
            //
            // var len = data.length;
            // for(var i=0;i<len;i++){
            //     data[i].src = "server";
            ...
            $.ajax({
                ...
                success:function(data){
                    var len = data.length;
                    for(var i=0;i<len;i++){
                        // getstatus状态不明 statusText
                        var dt={};
                        dt.src = "server";
                        dt.id = "SVR_FILE_"+i;
                        dt.rotation = 0;
                        dt.key = data[i].fileId;
                        dt.path = data[i].showPath;
                        fileQueue(dt);
                    }
                }
            });
    ...
    function addFile( file ){
        ...
        var $li = $('<li data-key="'+file.key+'"  data-src="'+file.src+'" data-sort="'+index+'" draggable="true" id="' + file.id + '" style="margin:5px;cursor:move;width:'+thumbnailWidth+'px;height:'+thumbnailHeight+'px;">' +
