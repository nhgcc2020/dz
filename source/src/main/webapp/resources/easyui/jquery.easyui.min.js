...
$("<div class=\"datagrid-header-check prettyck\"></div>").html("<input type=\"checkbox\"/><label></label>").appendTo(td);
}else{
if(col.field){
td.attr("field",col.field);
...
var tpTr=opts.finder.getTr(_752,"","allbody");
for(var m=0;m<tpTr.length;m++){
    var $this=tpTr.get(m);
    if($($this).find("div.datagrid-cell-check input[type=checkbox]").length>0){
        $($this).addClass("datagrid-row-checked");
    }
}
var bck=opts.finder.getTr(_752,"","allbody").find("div.datagrid-cell-check input[type=checkbox]");
...
cc.push(col.checkbox?"class=\"datagrid-cell-check prettyck\"":"class=\"datagrid-cell "+col.cellClass+"\"");
cc.push(">");
if(col.checkbox){
cc.push("<input  type=\"checkbox\" "+(_826.checked?"checked=\"checked\"":""));
cc.push(" name=\""+_828+"\" value=\""+(_829!=undefined?_829:"")+"\"><label></label>");
}else{
if(col.formatter){
...
var opts=_852.options;
    var _853;
if(opts.finder.getRows(_851)){
    _853= opts.finder.getRows(_851).length==0;
}else{
    _853=false;
}
 // _853=opts.finder.getRows(_851).length==0;
