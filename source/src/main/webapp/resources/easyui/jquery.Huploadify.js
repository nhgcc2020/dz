(function($){
	$.fn.Huploadify = function(opts){
		var itemTemp = '<div id="${fileID}" class="uploadify-queue-item">' +
				'<div class="cancel"><a class="delfilebtn"'+
        'href="javascript:void(0);">删除</a>'+
            '<a class="downfilebtn" href="javascript:void(0);"'+
        'style="">下载</a><a href="javascript:void(0);" class="uploadbtn">上传</a></div>'+
			'<span class="up_filename">${fileName}</span><span class="data"></span><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>';
		var defaults = {
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '<div id="attachFile-button" class="uploadify-button " style="cursor:pointer;height: 30px; line-height: 30px;margin-top:10px; width: 100px;"><span class="uploadify-button-text">';
				...
				_renderFile : function(file){
					...
					if(option.showUploadedSize){
						...
						$html.find('.up_filename').after("（"+F.formatFileSize(file.size)+"）");
						...
						xhr.onreadystatechange = function(e) {
							if(xhr.readyState == 4){
								if(xhr.status == 200){
                                    ...
                                    _this.find('#fileupload_'+instanceNumber+'_'+file.index).find(".delfilebtn").attr("did",xhr.responseText);
                                    _this.find('#fileupload_'+instanceNumber+'_'+file.index).find(".downfilebtn").attr("did",xhr.responseText);
									option.onUploadSuccess && option.onUploadSuccess(file,instanceNumber, xhr.responseText);
									//在指定的间隔时间后删掉进度条
									// setTimeout(function(){
									...
									option.onUploadError && option.onUploadError(file,instanceNumber, xhr.responseText);
								}
								option.onUploadComplete && option.onUploadComplete(file,instanceNumber,xhr.responseText);
