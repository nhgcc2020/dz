...
<div class="new_header">修改工资单</div>

<div class="row" style="margin: 0 0 10px 40px;line-height: 30px;">
    <div class="form-group">
        <label style="width: 70px;padding-right:0;" class="col-xs-1 control-label">经办人员 :</label>
...
fitColumns : true,
        ...
        <tr>
            ...
            return opeartionDel(index,row) }" width="50">操作
            </th>
            ...
            return inputTemplateWay(value,index,row.expenseType) }" width="50">费用分类
            </th>
...
<script>
    var pageSaveOrderConUrl;// 发送请求存储数据
    var customerListUrl = "/ydz/expensebook/listSpender";// 获取所有员工
    var listurl = "/ydz/expensebook/selectOneExpenseBook?expenseType=1&&expenseNo=" + expenseNo;
    var numberArr = [];
    var atter;
            ...
            success: function (data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        copy(list);
                        total(list);
                        $("#spenderKey").html(returnopt(data, 1)).val(list.spenderKey).trigger("change");
                        $("#spenderKey").select2({
    ...
    function copy(list) {
        atter = list
        $('#createOrder').datagrid({data: list.details})
    }
    ...
    function changeClients(list) {
        if (type == "update") {
            $(".new_header").text("修改费用");
            $(".btn-primary").text("保存");
            $("#orderDate").val(list.payDate);
            pageSaveOrderConUrl = "/ydz/expensebook/update";
        } else {
            $(".new_header").text("新建费用")
            $(".btn-primary").text("创建")
            $("#orderDate").val(new Date().Format("yyyy-MM-dd"));
            pageSaveOrderConUrl = "/ydz/expensebook/insert";
        }

        $("#expenseExplain").val(list.expenseExplain);
        var data = {
        ...
        $("#payType").html(returnopt(data, 2)).val(list.payType).trigger("change");
        $("#payType").select2({
    ...
    function total(list) {
        $('#heji').html(accounting.formatNumber(list.totalMoney, 2));
    }
    function opeartionDel(index, row) {
    ...
    var teams={};
    function req_teamsData() {   var query={settingClass :"02"};
    ...
    function saveOrder(o, url) {
        ...
        if ($("#spenderKey").val() == "" || $("#spenderKey").val() == null) {
            $("<div class='tips_contants'>").fadeIn(250).html("请填写经办人").delay(1500).fadeOut(250).appendTo('body');
            setTimeout(function () {
                ...
                if(i == readyData().details.length - 1 && sent == true){
                    $.ajax({
                            ...
                            success: function (callBack) {
                                var config = {
                                    menuUrl: "/openPage?pageName=expense_expense-wages-list"
                                };
    ...
    function readyData() {
        ...
        var order = {
            ...
            "comKey": atter.comKey,
            "expenseNo": atter.expenseNo,
            "industryId": atter.industryId,
            "expenseType":1
        };
        expenseBook = order;
