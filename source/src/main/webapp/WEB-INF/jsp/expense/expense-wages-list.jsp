...
<head>
    ...
    <script>
        ...
        var pageSetUrl="/ydz/expensebook/expenseTypeSetting";//getExpenseTypeForDropdown settingClass  01日常支出 02工资支出
        ...
        function similar(row) {
            ...
            var config = {
                path: "/ydz/expensebook/forward-update?expenseType=1&&expenseNo=" + row + "&&type=copyAdd",
            };
        ...
        function amend(row) {
            var App = window.parent.App;
            var config = {
                path: "/ydz/expensebook/forward-update?expenseType=1&&expenseNo=" + row + "&&type=update",
            };
            App.open(config.path, config);
        }
        function openTaxPoint() {
            var App = window.parent.App;
            var config = {
                path: "/ydz/expensebook/forward-add?expenseType=1"
            };
            App.open(config.path, config);
        }
        function delet(row) {
...
<div data-options="region:'north'"
     style="border:none !important; padding:10px 10px 9px;margin-right: 10px;overflow: hidden;">
    <div c-widget="ButtonGroup;">
        <div class="header">工资支出<span>记录人员工资、奖金、五险一金、福利费等；</span></div>
