...
<head>
    ...
    <style>
        ...
        .related_form_check,.related_form_check a{
            font-size: 12px;
        }
        .shark_contant {
            height: auto;
            width: 100%;
            background: -webkit-linear-gradient(top,#fff 5%,#f5f5f5 9%);
             padding-bottom: 0;
            overflow: hidden;
        }
    </style>
...
<div class="shark_contant">
    ...
    <div class="mui-content" style="background: transparent;">
        <div class="mui-content-padded related_form_content">
            <p style="color: #fff;">注册账号</p>
            <form id="signForm" class="mui-input-group related_form">
                <div class="mui-input-row related_form_row">
                    <input type="text" id="userId" name="userId" value="${sessionScope.sessionInvite.invitee}" readonly placeholder="请输入手机号">
                </div>
                <div class="mui-input-row related_form_row">
                    <input type="password" class="mui-input-password" id="password" name="password" placeholder="请输入密码">
                </div>
                <div class="mui-input-row related_form_codes related_form_prov_undline">
                    ...
                    <div><button id="verification">获取短信验证码</button></div>
                </div>
                <div class="related_form_check related_form_prov_undline" style="color: #666;">
                    <input value="Item 1" type="checkbox" id="related_form_prov_undline_input_check">
                ...
                <div class="mui-button-row related_form_sub">
                    <button type="button" class="mui-btn mui-btn-primary" disabled id="next_tap">下一步</button>
                </div>
            </form>
        </div>
        <div class="mui-content-padded related_form_content">
        <div style="text-align: center;position: relative;
    margin: 120px auto 20px; color:#8d8787;" class="desc-tit">
    ...
    display: block;
    height: 30px;
    font-size: 0.7rem;"><img style="    vertical-align: middle;
    width: 18px;
    margin-right: 8px;" src="/wechat/image/divide.png">来自伙伴推荐，下滑了解更多价值</p>
        <span style="position: absolute;
    top: 20px;
    left: 0;
    z-index: 0;
    width: 100%;
    border-bottom: 1px solid #ede8e8;"></span></div>

        </div>
       <%-- <div style="    background: url(/wechat/image/h1.png) no-repeat center #f5f5f5 ;
 width: 320px;height: 569px;
    background-size: 100%;
    margin: 0 auto;"></div>
        <div style="    background: url(/wechat/image/h2.png) no-repeat center #fff;
width: 320px;height: 569px;
    background-size: 100%;
    margin: 0 auto;"></div>
        <div style="    background: url(/wechat/image/h3.png) no-repeat  center #f5f5f5;
width: 320px;height: 569px;
    background-size: 100%;
    margin: 0 auto;"></div>
        <div style="    background: url(/wechat/image/h4.png) no-repeat center #fff;
width: 320px;height: 569px;
    background-size: 100%;
    margin: 0 auto;"></div>--%>
        <div style="
background: #f5f5f5;
text-align: center;
    margin: 0 auto;"><img style="width: 320px;height: 569px" src="/wechat/image/h1.png"></div>
        <div style="
        background: #fff;text-align: center;
    margin: 0 auto;"><img style="width: 320px;height: 569px" src="/wechat/image/h2.png"> </div>
        <div style="
        background: #f5f5f5;text-align: center;
    margin: 0 auto;"><img style="width: 320px;height: 569px" src="/wechat/image/h3.png"></div>
        <div style="
        background: #fff;text-align: center;
    margin: 0 auto;"><img  style="width: 320px;height: 569px" src="/wechat/image/h4.png"></div>
    </div>
...
<script>
    ...
    $("#next_tap").click(function(self){
        ...
        if(realName == undefined || realName == ""){
            $("<div class='tips_contants'>").fadeIn(250).html("真实姓名不能为空").delay(3000).fadeOut(250).appendTo('body');
            return false;
        };*/
        var password = $("#password").val();
        if(password == undefined || password == ""){
            $("<div class='tips_contants'>").fadeIn(250).html("密码不能为空").delay(3000).fadeOut(250).appendTo('body');
            return false;
        };
        $.post("<c:url value="/invite/com/signUpCom"/>",$("#signForm").serialize(),function(callback){
            //var callback = eval("("+data+")");
            if(callback.state){
                $("#next_tap").text("进入下一步...");
                setTimeout(function(){
                    window.location.href="<c:url value="/invite/com/createComPage"/>"
                },1000);
            }else{
    ...
    $("#verification").click(function(){
        ...
        $(this).countdown(120);
        $.get("<c:url value="/tool/sendVerCode"/>",{phone:phone},function (data) {});
    });  //调用倒计时插件
