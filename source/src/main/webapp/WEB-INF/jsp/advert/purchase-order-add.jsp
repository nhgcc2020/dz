...
<head>
    ...
    <style>
        .ui.calendar.mild .date-table-wrapper .date-table td.selected div.label{background:#2e8ae6}
        .ui.calendar.mild .date-table-wrapper .date-table>tbody>tr>td:first-child,.ui.calendar.mild .date-table-wrapper .date-table>tbody>tr>td:last-child{background:rgba(46,138,230,.1)}
        .ui.calendar.mild .date-table-wrapper .date-table td:hover:not(.selected) div.label{background:rgba(46,138,230,.5)}
        .ui.positive.button,.ui.positive.buttons .button{background-color:#2185D0!important;color:#FFF;text-shadow:none;background-image:none}
        ul li label{width:250px;display:inline-block;height:22px;line-height:22px;overflow:hidden}
        .ui.button{padding:0 30px}
        .moneyInput input{line-height:initial!important;height:initial!important}
        .ui.input.error input{border-width:1px!important;border-radius:2px!important}
        .tips_contants{height:80px;line-height:80px;width:300px;text-align:center;background:rgba(0,0,0,.8);color:#fff;position:fixed;top:50%;margin-top:-150px;left:50%;margin-left:-150px;border-radius:5px;display:none;z-index:9999}

        .add_tips{font-size:12px;cursor:pointer;position:relative;padding:5px 0 10px 26px;color:#999;width:100px}
        .add_tips:hover,.add_tips:hover i{color:#2e8ae6}
        .add_tips i{position:absolute;left:0;font-size:1.8rem;color:#999;top:4px}
        .secondary,.secondary i{color:#2e8ae6}
        #priceTable{overflow:auto;height:390px}
        .ui.list-view .items{
            max-width: 215px;
        }
...
<body class="modal-page" ontouchstart="">
...
<div class="ui form">
    <div class="inline fields">
        <div class="field">
            ...
            <%--<div id="clientsDp" style="width:200px"></div>--%>
            <select  id="clientsDp" style="width:200px" onchange="changeClients(this)" placeholder="选择供应商">
            </select>
        </div>
        <div class="field">
            <label style="display: inline-block;margin-left: 30px;">联系姓名</label>
            <%--<div id="contacterDp" style="width:150px"></div>--%>
            <select disabled id="contacterDp" style="width:150px" >
            </select>
        </div>
        <div class="field">
    ...
    <div class="inline fields">
        <div class="field">
            <label>项目名称</label>
            <div c-widget="input; placeholder:散单生意可以概要填写，以备后续查询; bind:purchaseOrder.caseDesc;width:955" style="margin-right: 46px"></div>
        </div>
    </div>
</div>
<!--订单分类 start-->
<div id="tagsWrap" style="display: none;">
    添加分类订单
    <div c-repeat="tag in tags"
    <%--c-class="checked:isRangeChecked(tag.modelName) on ranges.*"--%>
         class="tag-range current checked">
        <div c-widget="checkbox; bind:{{'ranges.'+tag.modelName}}; checked:true; label:{{tag.label}}; userData:{{tag}};disabled:true;"
             class="rotating checked" ></div>
    </div>
</div>
<!--订单表格 start-->
    ...
    <div class="h-box">
            ...
            <div class="v-box">
                <div class="flex-box">
                    <div class="remark-container">
                        <div id="remarkBtn" class="add_tips" c-onclick="showRemark(this)" ><i class="iconfont_new">&#xe6ca;</i>添加备注</div>
        ...
        <div class="box right-summary">
            ...
            <div style="display:none" class="h-box sales-box">
                ...
                <div class="flex-box">
                    <div>
                        <div class="moneyInput"  c-widget="input; icon:percent;width:80px;bind:order.sales;" c-watch="discountChange on order.sales"></div>
                    </div>
                </div>
                <div class="box price"><span c-bind="formatNumber(purchaseOrder.discount,'-#,##0.00')"></span>
                </div>
            </div>
            <div style="display:none" class="h-box minus-box">
            ...
            <div class="h-box">
                ...
                <div class="box price"><span
                        c-bind="bindChange(purchaseOrder.price - purchaseOrder.discount - purchaseOrder.preferential,'totalPrice')"></span>
                </div>
            </div>
            ...
            <div style="display:none" class="h-box earnest-box">
                ...
                <div class="flex-box">
                    <div>
                        <div class="moneyInput" c-widget="input; width:80px;  bind:purchaseOrder.depositRequested"></div>
                    </div>
                </div>
                <div class="box price" c-bind="formatNumber(purchaseOrder.depositRequested,'-#,##0.00')" ><span></span></div>
            </div>
            <div class="h-box">
    ...
    <div class="h-box actions">
        ...
        <div class="box">
            <div class="ui segment basic">
                <c-button caption="创建订单并付款" color="orange" onclick="ifSaveCash"></c-button>
...
<div class="ui  modal " id="createOrderPop" style="width: 660px;height: 480px;">
    ...
    <div style="    width: 500px;height: 30px;line-height: 30px;margin: 20px auto;border: 1px solid #ffcc00;font-size: 12px;color: #666;background-color: #ffffcc;text-align: center;">一次付清货款，“应付金额”即“实付金额”，系统直接生成已付款对账单</div>
    ...
    <p style="width: 360px; margin: 70px auto 20px; height: 30px; position: relative; text-align: right;">
        ...
        <i style="    color: #2e8ae6;font-size: 12px;margin-left: 10px;position: absolute;right: -66px;font-style: normal;cursor: pointer;" onclick="addShuiDian(this)">添加税点</i>
    </p>
    ...
    <div id="shuiDianCtrl" style="border: 1px solid rgb(204, 204, 204);width: 400px;padding: 20px 20px 0px;margin: 0px auto 20px;display: none;">
        <p style="    width: 360px;margin: 0 auto;height: 40px;"><span style="float:left;color:#666;font-size:14px;text-indent: 10px;">税率：</span><span style="float:right;color:#666;font-size:26px;">
                    <input type="text" style="    font-size: 14px;border: 1px solid #ccc;text-indent: 10px;border-radius: 2px;width: 40px;height: 20px;vertical-align: middle;"
        ...
        <p style="width: 360px;margin: 0 auto;height: 40px;text-align: right;"><span style="float:left;color:#666;font-size:14px;text-indent: 10px;">税额：</span><span  id="rateAmount" style="color:#666;font-size:16px;">..</span></p>
    </div>

    <p style="    width: 360px;border-top:1px solid #ccc; margin: 0 auto 74px; padding-top: 30px; height: 30px;" id="payAmoutBd"><span style="float:left;color:#666;font-size:14px;font-weight: 700;">应付金额：</span>
...
<script src="/resources/business/dzorder/purchase-order/add.js?v=4458c673ea"></script>
<script>
    cola(function (model) {
        //折扣优惠
