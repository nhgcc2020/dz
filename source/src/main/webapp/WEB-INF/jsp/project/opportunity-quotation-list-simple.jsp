...
<head>
    ...
    <%--简单实现的下拉框方式--%>
    <style>
        .prettyck input:checked~label:after{
            top:13px;
        }
        .prettyck label:before{
            top:10px;
...
<div data-options="region:'north'" style="border:none !important;padding:10px;overflow: hidden;">
    ...
    <ul class="nav nav-tabs" id="navCondition">
        ...
        <li style="position:relative;" data-s="0"  class="order2 smcolor"><a href="#">选择报价</a>
        </li>
...
<div id="pageToolbar"
     style="font-weight: bolder;position: fixed;bottom:10px;left:45%; line-height: 50px; overflow: hidden;">

    <button type="button" class="btn btn-primary pull-right  "
    style="margin-right:15px;height:30px;line-height:1;"
    onclick="oppColumnFun.openReceivablesBar()">选择报价
    </button>
...
//            window.location.href = turl;
        ...
        openReceivablesBar: function () {
            ...
            window.parent.relateOpp(arr.join(","));
...
//                var r=row.caseDesc==""?"无概要":row.caseDesc;
                return '<div data-id=' + row.oppoNo + ' data-did=' + row.industryId + ' href=\'##\' onclick=openDt(this)>' + value + '</div>' ;
            }
        });
        ...
        arrclo.push({
            field: 'partnerShortname', width: 100, align: 'center', title: '客户', formatter: function (value, row) {
...
//        var sum = '报价总金额(元)：<span style="color: red;margin-right: 15px;" id="orderPrice">0.00</span>';
//        var odcount = '订单数(笔)：<span style="color: red;margin-right: 15px;" id="order2PagerNum">0</span>';
//        var status = $("#navCondition").find(".smcolor").attr("data-s");
