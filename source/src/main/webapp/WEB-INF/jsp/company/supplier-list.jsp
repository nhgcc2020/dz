...
<div  data-options="region:'north'" style="border:none !important;padding:10px 10px 0;margin-right: 10px;overflow: hidden;" >
    ...
    <ul class="nav nav-tabs" id="navCondition">
        <!--<li style="position:relative;" >     <a href="#">待对账/付款供应商</a><span id="message-num-circle0" style="
        line-height: 18px;
    text-align: center;
    height: 18px;
    min-width: 18px;
    ...
    font-size: 12px;">0</span></li>-->
        <li style="position:relative;" class="active" number=""><a href="#">全部供应商</a>     <!--<span id="message-num-circle1" style="
       line-height: 18px;
    text-align: center;
    height: 18px;
    min-width: 18px;
    ...
    padding:0 2px;
    font-size: 12px;">0</span>--></li>
        <li style="position:relative;" number="1">     <a href="#">好友供应商</a></li>
        <li style="position:relative;" number="-1">     <a href="#">非好供应商</a></li>
        <li class="block"></li>
    </ul>
...
可查看全部内容<br>
3.  <span style='color:#ff9900'>超过6个月待对账订单：</span>订单日期（非制单日期）至今已过6个月的
订单；例：4月7日订单，至10月8日即被计入 " >操作技巧<span  class="iconfont_new" style="position: relative;top:-1px;vertical-align: middle;font-size: 20px">&#xe6b8;</span></div>
    <div class="h10"></div>
    ...
    font-size: 12px;
    padding-right: 30px;">
                        <!--<span class="input-group-btn iconfont_new" id="searchBtn"  style="        cursor: pointer;
    position: absolute;
    top: 2px;
    width: 20px;
    right: 8px;
...
<div id="regionCon"  data-options="region:'center'" style="border:none !important;padding:10px;width: 100%;" >
    <table id="order"  style="width: 100%;" class="easyui-datagrid order"
        ...
        <tr>
            ...
            <th data-options="field:'partnerStatus',width:200,resizable:false,align:'center',formatter: function(value, row) {
...
<script>
    ...
    function  showModPartner(partnerMod) {
        ...
        //   /openPage?pageName=company_customer-list&type=1
...
//        $("#orderProcedure").addClass(" flipOutY").removeClass("flipInY");//css({display:'none'});
    ...
    /* openSendMsg: function (partner) {
     var partnerAttr = partner.get("partnerAttr");
     //1 公司
     if (partnerAttr == "1") {
     model.set("inviteCom", partner.get("partnerName"));
     } else {
     model.set("inviteCom", "个人用户");
     }
     model.set("sendPhone", partner.get("tel1"));
     model.set("inviteName", partner.get("contacterName"));
     model.set("sendObj", partner);
     cola.widget("sendMsgDiv").show();
     },*/
...
//            $("<div class='tips_contants'>").fadeIn(250).html("没有查询到数据").delay(1000).fadeOut(250).appendTo('body');
    ...
    function reload(status) {
        var url='system/partner/supplierList';
