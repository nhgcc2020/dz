...
<head>
    ...
    <script >
        ...
        var pageSaveOrderConUrl="/order/installation/insert"; //保存订单
        var detailUid="${detailUid}";
        var pageDetailConConUrl="/order/installation/listInstallStandingByDetailForAdd?detailUid=" + detailUid;
        var globDataGrid = [
        ...
        var installionAddFun={
            ...
            $.ajax({
                ...
                success:function(data){
                    $("#clientsDp").html( installionAddFun.returnopt(data,1)).val("");
                    $("#clientsDp").select2({
                        templateResult: installionAddFun.formatState
                    });
                    $("#clientsDp").on("change", function (e) { installionAddFun.req_teamsData() });
                },
                error:function(){
                }
            });
            ...
            var order= {
                ...
                "discount": accounting.formatNumber($("#discountVal").val(),2, ""),//折扣金额
                "preferential": $("#preferentialVal").val(),// 减免金额
                "installationPrice": accounting.formatNumber($("#totalPriceVal").val(),2, ""),// 订单金额
                "prepayPrice": accounting.formatNumber($("#depositRequested").val(),2, ""),//预付金额
                "payablePrice": accounting.formatNumber($("#receivablesMoneyVal").val(),2, "")//应付金额
            ...
            $.ajax({
                ...
                success: function (callBack) {
                    if (callBack.code == '1') {
                        ...
                        $(".max-hidden", window.parent.document).find("li[data-url='/openPage?pageName=assignment_installation-list']").click().addClass("active");
                    }else{
...
//                '<span><img src="/' + state.element.value.toLowerCase() + '.png" class="img-flag" /> ' + state.text + '</span>'
            ...
            if(partnerKey == undefined || partnerKey==""){
                $("<div class='tips_contants'>").fadeIn(250).html("请选择供应商").delay(2000).fadeOut(250).appendTo('body');
                return false;
            }
            $(".window-mask").show();
            $("#topBarPR").show();
...
//                            "requirements": currentItem.requirements,
            ...
            return '<input class="autocomplete" readonly style="width: 100%;" value="'+v+'" type="text"><i class="iconfont_new" style=" cursor: default;  font-size: 14px;right: 1px;line-height:40px;-webkit-transform: scale(.5);position: absolute;color: #ccc;">&#xe68d;</i>'
        },
...
<div class="header">
    <img src="/images/picture.png" alt="">安装工单
</div>
    ...
    <div class="h-box actions">
        ...
        <div class="box">
            <div class="btnGrp ui segment basic">
                ...
                <button type="button" style="width: 100px" userdata="1" onclick="installionAddFun.saveAll(this)" class="btn btn-primary">创建工单</button>
