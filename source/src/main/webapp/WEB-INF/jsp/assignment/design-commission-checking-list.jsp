...
<title>设计派工提成结算</title>
        ...
        <script>
            ...
            var pageListSaleConUrl="/order/assignment/listDesignEmployee"; //销售代表下拉框
            var designPageCheckFun = {
                ...
                saleManDp:  function (){
                    $.ajax({
                        ...
                        success:function(dt){
                            ...
                            $(".recpayNameList").html(designPageCheckFun.returnopt(dt.taskEmployees, 3));
...
//                '<span><img src="/' + state.element.value.toLowerCase() + '.png" class="img-flag" /> ' + state.text + '</span>'
                ...
                arrFunDetail: function () {
                    ...
                    arrclo.push({field: 'commissionsCheckingNo',  width: 140, align: 'center', title: '结算单号',  formatter: function (value, row) {
//                        var r = row.caseDesc == "" ? "无概要" : row.caseDesc;
                        return '<a data-id=' + row.commissionsCheckingNo + ' data-did=' + row.industryId + ' href=\'##\' onclick=designPageCheckFun.openDt(this)>' + row.commissionsCheckingNo + '</a>';
                    }});
                    ...
                    arrclo.push({field: 'taskUserName', width: 80,align: 'center', title: '设计师'});
...
//                    });
                    ...
                    if(status=="12"){
                        ...
                        arrclo.push({field: 'balanceRemarks', width: 180, align: 'center', title: '差额说明'});
                    }
                ...
                onLoadSuccess: function (data) {
                    if (data.rows.length > 0) {
                        ...
                        $("#order2PagerPrice").text(accounting.formatNumber(data.bottomInfo.sumTotalPrice, 2));
...
//                    });
                    ...
                    $.ajax({
                        ...
                        success:function(callback){
                            if(callback.code =="1"){
                                ...
                                designPageCheckFun.closeReceivablesBar();
                ...
                showMessageDialog: function (url, title, width, height, shadow) {
                    ...
                    setTimeout(function () {
                        ...
                        if ($("#msgwindow").hasClass("activ")) {
                            designPageCheckFun.msg(content);
                        } else {
                ...
                openDt: function (obj) {
                    var url = "/ydz/commission/forward-design-commissionchecking-detail?commissionsCheckingNo=" + $(obj).attr("data-id") ;
                    designPageCheckFun.showMessageDialog(url, "", "84%", "100%", true);
                }
            }   //本页面的交互方法
            $(function () {
                ...
                $("#navCondition").on("click", "li", function () {
                    if($(this).index()>1){
                        ...
                        window.location.href="/openPage?pageName=assignment_design-commission-list&status="+$(this).index();
                    }
                })
                $(".createInstaller,#modifyOrder").click(function () {
                    ...
                    if(selectItems.length == 0){
                        ...
                        $.mytoast({text:"请选择一笔需要结算的对账单"});

                        return false;
                    }
                    if(selectItems.length > 1){
                        $.mytoast({text:"不可同时结算多笔对账单"});
                    ...
                    $(".partnerShortnameCls").text(selectObj.taskUserName);
...
//                $("#dateDat").html(designPageCheckFun.returnopt(orderUtil.dateConfig, 2)).change(function () {
...
//                });
                ...
                designPageCheckFun.saleManDp();
                $.ajax({
                    url:pageCustomerUrl,
                    dataType: "json",
                    success: function (data) {
                        $("#customerDat").html(designPageCheckFun.returnopt(data, 1));
                        $("#customerDat").select2({
//                        maximumInputLength:2,
                            templateResult: designPageCheckFun.formatState
                        });
                ...
                $(".recpayAccountList").html(designPageCheckFun.returnopt(globeConfigData.recpayAccountList, 4));
            });//本页面加载完成初始化数据
            // 公共方法
            function reloadCopy(status) {
                ...
                var arrclo = designPageCheckFun.arrFunDetail();
                $('#order').datagrid({
                    url: url,
                    fit: true,
                    onLoadSuccess: designPageCheckFun.onLoadSuccess,
                    pagination: true,
                    pageSize: pageSize,
                    pageList: pageList,
                    columns: [arrclo],
...
">订单周期：</label>&nbsp;
    ...
    <span style="  position: absolute; top:15px;right:1px;display: block;height:30px;width: 30px; cursor: pointer;" onclick="designPageCheckFun.closeTaxPoint()"><img src="../../images/close_user.png" alt="点击关闭"></span>
    <div style="width: 560px;margin: 0 auto;position: relative">
        ...
        <div style="display: inline-block;vertical-align: middle;width: 184px;">
            <img class="comImgTran" style="margin: 38px auto 38px;float:left;width: 50px;height: 50px;border-radius: 40px;border:1px solid #ccc;padding:3px;" src="/images/shareDesign.png"><em class="partnerShortnameCls" style="margin-top: 45px;margin-left: 10px;display: inline-block;width: 110px;overflow: hidden;font-weight: 700;text-overflow: ellipsis;height: 20px;word-wrap: break-word;font-size: 14px;"></em><br><span class="contactNameCls" style="
    margin-left: 10px;
    color: #999;
"></span>
        </div>

        <div style="display: inline-block;width: 154px;text-align: center;">
            <label style="
    font-weight: normal;
    color: #999;
    font-size: 14px;
...
"></span>
        ...
        <div style="display: inline-block;width: 214px;">
            <label style="
    font-weight: normal;
    color: #999;
    font-size: 14px;
...
"></span>
        ...
        <div style="width:552px;margin:35px auto;">
            <div style="width:215px;display: inline-block;position:relative;margin-right: 117px;text-align: right;margin-bottom: 20px;height: 34px;line-height: 34px;"><label style="float: left;font-size: 14px;color: #333;font-weight: normal;">实付金额：</label>
                <input placeholder="0.00" type="number" name="recpayMoney" data-s='select' onkeyup="designPageCheckFun.recpayKeyUp(this)" style="width:120px;    text-align: right;line-height: 18px;
...
"><i class="iconfont_new" style="font-size: 18px;">&#xe6a6;</i></span>
    ...
    <div style="clear: both;position:absolute;bottom:30px;right: 105px;">
        <button type="button" class="btn " onclick="designPageCheckFun.closeTaxPoint(this)" style="width:100px;">取消</button>
        <button type="button" class="btn btn-primary sureReceivables" style="margin-left:15px;width:100px;font-size:12px;" onclick="designPageCheckFun.sureReceivables(this)" id="orderMake">添加结算记录
        </button>
    </div>
</div>
        ...
        <form role="form">
            <div class="row" style="margin:0;">
                ...
                <div class="col-xs-2 " style="width:120px;padding:0 5px">
                    <select placeholder="" class="form-control input-xs" id="saleManDp" >
                    </select>
                </div>
                <%--<div class="col-xs-2" style="width:120px;padding:0 5px">--%>
