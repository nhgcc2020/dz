...
<%--<button type="button" onclick="order_detail.updateOrder()" class="btn updateOrder "--%>
...
<script id="tpl-detail" type="text/html">
    ...
    <div class="ui form" style="position:relative;">
        <div class="order">
            <p>安装对账单号 : <span id="orderNum">{{order.installationcheckingNo}}</span></p>
        </div>
    ...
    <div class="ui form">
        ...
        <table class="tbWk" style="width:100%;">
            ...
            <col width="12%"/>
            <thead>
            <th style="text-align: center">工单日期</th>
            <th>工单号</th>
            <th>业务内容</th>
            <th>规格(m)</th>
            <th>面积(㎡)</th>
            ...
            <tr>
                <td>{{item.installationOrderDate}}</td>
        ...
        <div class="total-drive">
            ...
            <div style="position:relative;">
                <ul class="total_right">
                    ...
                    <li class="hide payablePriceCls" style="margin-bottom:8px;">应&#x3000;&#x3000;付 :<span
                    >{{order.payablePrice | formatNumber}}</span></li>
                    <li class="hide paidUpCls" style="margin-bottom:8px;">实&#x3000;&#x3000;付 :<span
                    >{{order.paidUp | formatNumber}}</span></li>
                </ul>
                <div style="position:absolute;left:0;bottom:0;">
                    <p class="show_address">客户地址 ：<span class="province{{order.installationcheckingNo}}">{{bottomComInfo.partnerProvince}}</span>
                        <span class="city{{order.installationcheckingNo}}">{{bottomComInfo.partnerCity}}</span> <span
                                class="country{{order.installationcheckingNo}}"
                        >{{bottomComInfo.partnerCountry}}</span>
                        <span class="address{{order.installationcheckingNo}}">{{bottomComInfo.partnerAddress}}</span></p>
    ...
    <div class="Customer-address">
        ...
        通讯地址 : <span>
                    ...
                    <b id="sheng1{{order.installationcheckingNo}}"></b>
                    <b id="shi1{{order.installationcheckingNo}}"></b>
                    <b id="xian2{{order.installationcheckingNo}}"></b>
...
<script type="text/javascript">
    var pageUpdateOrder = "/ydz/salesorder/updatePage?salesNo=";// 修改订单请求url
    var pageCancelOrder = "/ydz/salesorder/cancelSalesOrder";// 作废订单请求url

    var pageshareUrl = "/order/installationchecking/shareUrl?orderNo=${param.installationcheckingNo}";
    var installationcheckingNo = '${param.installationcheckingNo}';
    $(function () {
        try {

            //打印
            $("#print").click(function () {
                window.parent.parent.open("/print/installation-checking-list-detail-print.html?installationcheckingNo=${param.installationcheckingNo}");
            });

        } catch (e) {

        }
        order_detail.initTbData(installationcheckingNo);

    })

    //    页面方法的主体
    var order_detail = {
        initTbData: function (installationcheckingNo) {
            $.ajax({
                url: "/order/installationchecking/checkingOrderAndDetails?installationcheckingNo=" + installationcheckingNo,
                type: 'post',
                ...
                success: function (bothData) {
                    ...
                    $(".depositRequestedCls").text(bothData.order.depositRequested);

                    $("#totalPrice").text(accounting.formatNumber(bothData.order.totalPrice, 2));
                    ...
                    if (bothData.order.status == '09') {
                        $(".payablePriceCls").removeClass("hide");
                    }
                    if (bothData.order.status == '10') {
                    ...
                    if (bothData.bottomComInfo.partnerProvince != "" || bothData.bottomComInfo.partnerCity != "" || bothData.bottomComInfo.partnerCountry != "" || bothData.bottomComInfo.partnerAddress != "") {
                        ...
                        order_detail.pccJson(installationcheckingNo, bothData.bottomComInfo.partnerProvince, bothData.bottomComInfo.partnerCity, bothData.bottomComInfo.partnerCountry);

                    } else {
                        $(".show_address").hide()
                    }
                    order_detail.initData(installationcheckingNo, bothData.bottomComInfo.comProvince, bothData.bottomComInfo.comCity, bothData.bottomComInfo.comCountry);
                }
            })

        },

        pccJson: function (installationcheckingNo, province, city, country) {
            $.each(ProvinceJson, function (key, val) {
                if (val.provinceId == province) {
                    $(".province" + installationcheckingNo).html(val.provinceName);return false;
                }
            });
            $.each(CityJson, function (key, val) {
                if (val.provinceId == province && val.cityId == city) {
                    $(".city" + installationcheckingNo).html(val.cityName);return false;
                }
            });
            $.each(CountryJson, function (key, val) {
                if (val.cityId == city && val.countryId == country) {
                    $(".country" + installationcheckingNo).html(val.countryName);return false;
                }
            });

        },

        initData: function (installationcheckingNo, sheng, shi, xian) {
            //通讯地址
            $.each(ProvinceJson, function (key, val) {
                if (val.provinceId == sheng) {
                    $("#sheng1" + installationcheckingNo).html(val.provinceName);return false;
                }
            });
            $.each(CityJson, function (key, val) {
                if (val.provinceId == sheng && val.cityId == shi) {
                    $("#shi1" + installationcheckingNo).html(val.cityName);return false;
                }
            });
            $.each(CountryJson, function (key, val) {
                if (val.cityId == shi && val.countryId == xian) {
                    $("#xian2" + installationcheckingNo).html(val.countryName);return false;
                }
            });
        },
